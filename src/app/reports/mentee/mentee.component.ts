...
export class MenteeComponent implements OnInit {
  ...
  getMenteeList(){
    this.restApi.getMethod('getMenteeReportList').subscribe((resp:any) => {
  ...
  filterList(event,type){
    /* var result = this.tofilter.filter(function(item){
      if(item[type].toLowerCase().indexOf(event.target.value) > -1){
        return item;
      }
    }) */
    var result;
  ...
  valFilter(){
    var el = Array.from(document.getElementsByClassName('filter-fld') as HTMLCollectionOf<HTMLElement>);
    /* var result = this.tofilter.filter(function(item,key){
      for(var i=0; i<el.length ; i++){
        if((el[i] as HTMLInputElement).value != ''){
          if(item[el[i].id].toLowerCase().indexOf((el[i] as HTMLInputElement).value) > -1){
            return item;
          }
        }
      }     
  ...
  exportData() {
    ...
    link.setAttribute("download", "mentee-report.csv");
